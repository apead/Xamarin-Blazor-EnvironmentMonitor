...
namespace XamEnvMonitor.Droid.Callbacks
{
    public class WeatherDynamicSensorCallback : SensorManager.DynamicSensorCallback
    {
        ...
        public override void OnDynamicSensorConnected(Sensor sensor)
        {
            ...
            else if (sensor.Type == SensorType.RelativeHumidity)
            {
                _context.SensorManager.RegisterListener(new HumidityListener(), sensor,
                    SensorDelay.Normal);
    ...
    public class HumidityListener : Java.Lang.Object, ISensorEventListener
    {
        public static string Tag = typeof(MainActivity).FullName;
        ...
        public void OnSensorChanged(SensorEvent e)
        {
            ...
            var diff = Math.Abs(_lastHumidity - value);

            if (!(diff > _updateThreshold)) return;
            Log.Debug(Tag, "sensor changed: " + value);
            _lastHumidity = value;
            PubSubHandler.GetInstance().Publish(new HumidityMessage(this, value));
        }
    }
